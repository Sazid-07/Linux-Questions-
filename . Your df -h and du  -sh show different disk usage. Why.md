...
# ✅ **Why `df -h` and `du` / `dush` Show Different Disk Usage**

## **Simple Answer (Real-Time Admin Style)**

* **df -h** → filesystem level space usage (from *block layer*).
* **du / dush** → directory/file level usage (from *user-space directory tree*).
...
# 🧨 **Top 6 Real-Time Reasons for Difference (with examples)**

---

## **1️⃣ Deleted files still held by running processes**
...
But application is still writing to it → inode open hai.
...
File deleted → **0 bytes** (or small size).
...
👉 **Fix:** restart application or truncate the open file:
...
## **2️⃣ Mount point overlay (mount hiding actual files)**

**Scenario:**
Before mounting:

```
/data/    → contains 5GB old files
```
...
* **df -h → shows mounted disk (example 500GB)**
* **du /data → shows new empty directory**
...
## **3️⃣ Sparse files (logical size ≠ physical size)**

Example: VM images, database files.

### `du` shows:

Large logical size → e.g., 100GB file
...
## **5️⃣ Files in special FS (tmpfs, overlayfs, docker layers)**
...
## **6️⃣ Reserved blocks (EXT4 reserve for root)**

EXT4 reserves 5% blocks for root.

* `df -h` shows less available space
* `du` doesn’t count reserved blocks at all.
...
# 🔥 **Most Real-Time Practical Reason**
...
→ Deleted log files still being used.
